...
namespace ajy::memory::threadlocal
{
	...
	std::size_t ObjectPool<T, Args...>::IndexAllocator::acquire(void) noexcept
	{
		...
		{
			std::fprintf(stderr, "ajy::memory::threadlocal::ObjectPool::IndexAllocator::acquire() failed: [Code: %d] %s\n", error.code().value(), error.what());
			std::terminate();
		}
	}
	...
	void ObjectPool<T, Args...>::IndexAllocator::release(std::size_t index) noexcept
	{
		...
		{
			std::fprintf(stderr, "ajy::memory::threadlocal::ObjectPool::IndexAllocator::release() failed: [Code: %d] %s\n", error.code().value(), error.what());
			std::terminate();
		}
		catch (const std::bad_alloc &error)
		{
			std::fprintf(stderr, "ajy::memory::threadlocal::ObjectPool::IndexAllocator::release() failed: %s\n", error.what());
			std::terminate();
		}
	}
