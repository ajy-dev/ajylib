/**
 * File: net_server.cpp
 * Path: ajylib/source/network/windows/iocp/net_server.cpp
 * Description:
		...
		, max_sessions(0)
	{
		if (!this->logger)
		{
			...
			else
			{
				std::fprintf(
					stderr,
					"ajy::network::windows::iocp::NetServer::NetServer() failed: could not acquire logger \"%.*s\"\n",
...
clean_wsa:
	...
	void NetServer::stop(void) noexcept
	{
	...
	bool NetServer::disconnect(SessionID id) noexcept
	{
		Session *session;
	...
	bool NetServer::send_packet(SessionID id, std::shared_ptr<Packet> packet) noexcept
	{
		...
		packet->build_header(this->code, this->generate_random_key());
		packet->encode(this->fixed_key);
		...
		while (server->running.load(std::memory_order_acquire))
		{
			...
			if (overlapped == &session->recv_overlapped)
			{
				...
				try
				{
					...
					while (true)
					{
						std::byte header[Packet::HEADER_SIZE];
						std::uint16_t payload_length;
						std::size_t total_size;

						if (session->recv_buffer.get_used_size() < Packet::HEADER_SIZE)
							break;
						session->recv_buffer.peek(header, Packet::HEADER_SIZE);
						...
						std::memcpy(&payload_length, header + Packet::LEN_OFFSET, Packet::LEN_SIZE);
	...
	NetServer::SessionID NetServer::pack_session_id(std::uint32_t index, std::uint32_t generation) noexcept
	{
	...
	std::pair<std::uint32_t, std::uint32_t> NetServer::unpack_session_id(SessionID id) noexcept
	{
		static constexpr unsigned int GENERATION_SHIFT = 32;
	...
	std::uint32_t NetServer::calculate_tps(
	...
	std::uint8_t NetServer::generate_random_key(void) noexcept
	{
	...
	NetServer::Session *NetServer::acquire_session(SessionID id) noexcept
	{
	...
	void NetServer::release_session(Session *session) noexcept
	{
	...
	bool NetServer::recv_post(Session *session) noexcept
	{
		WSABUF wsabuf[2];
		DWORD flags;
		DWORD recv_bytes;
	...
	void NetServer::send_post(Session *session) noexcept
	{
	...
	void NetServer::log_winapi_error(const char *func_name, DWORD error_code, utility::Logger::LogLevel level) noexcept
	{
		LPSTR buffer;
