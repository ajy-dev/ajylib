...
50×50 섹터 격자형태의 맵 안에 수많은 클라이언트가 존재하며, 클라이언트가 채팅을 보내면, 서버는 그 채팅을 자신을 포함한 3×3 섹터에 브로드캐스트합니다.

섹터 이동은 단순성을 위해 서버가 아니라 클라이언트 권위를 선택했습니다. 섹터 이동 자체는 별도의 게임 서버에서 이루어진다 가정하고 클라이언트가 자신 위치를 채팅 서버에 전달합니다.
