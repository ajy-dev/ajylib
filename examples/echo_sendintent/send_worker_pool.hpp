 ...
 * Path: ajylib/examples/echo_sendintent/send_worker_pool.hpp
 * Description:
 *	Takes what to send, not a built packet, and does the building and the
 *	sending on its own threads.
 * Note:
 *	Measurement scaffolding. Deciding what to send needs group state and so
 *	must stay serial; turning that decision into bytes does not. This pool
 *	receives the decision as plain fields, so allocation, serialization and
 *	the send call all leave the group thread.
