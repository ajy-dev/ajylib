...
int main(void)
{
	...
	logger_index = ajy::utility::Logger::create("echo_with_group");
	...
	EchoWithGroupServer server("echo_with_group");
	...
	console.register_command(
		...
		[&server](std::istringstream &args)
		{
			...
			std::printf("Auth: %u", server.get_auth_session_count());
			for (i = 0; i < server.get_echo_group_count(); ++i)
	...
	console.register_command(
		...
		[&server](std::istringstream &args)
		{
			...
			std::printf("Auth: %zu", server.get_auth_job_pool_in_use());
			for (i = 0; i < server.get_echo_group_count(); ++i)
	...
	std::printf("EchoWithGroupServer management console. Type 'help' for commands, 'exit' to quit.\n");
	console.run();
