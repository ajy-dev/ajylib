 ...
 * Path: ajylib/examples/login_gated_chat/chat_server/chat_server.hpp
 * Description:
 ...
 * 	Registers with the login server over a separate SS channel (LoginLink).
 * Note:
...
public:
	...
	void kick_account(std::int64_t account_no) noexcept;
...
private:
	static constexpr std::size_t SESSION_KEY_SIZE = 64;
	static constexpr std::size_t HEX_PREFIX_BYTES = 8;
	...
	struct Job
	{
		union Target
		{
			SessionID session_id; // ClientJoin, ClientLeave, Recv
			std::int64_t account_no; // KickAccount
		};
		...
		Job(JobType type, Target target, std::shared_ptr<Packet> packet = nullptr) noexcept;
	};
	...
	void handle_kick_account(std::int64_t account_no) noexcept;
	void check_heartbeat_timeout(void) noexcept;

	void handle_req_login(Player *player, Packet *packet) noexcept;
	bool claim_session(std::int64_t account_no, const std::uint8_t *session_key) noexcept;
	void clear_session_state(std::int64_t account_no) noexcept;
	void clear_all_session(void) noexcept;

	static void to_hex_prefix(char *dst, const std::uint8_t *key, std::size_t len) noexcept;
