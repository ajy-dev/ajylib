 ...
 * Path: ajylib/examples/login_gated_chat/chat_server/chat_server.cpp
 * Description:
...
namespace
{
	 ...
	 * from ChatServerConfig, the account number is at most 20 characters.
	 */
	constexpr std::size_t REDIS_KEY_SIZE = 64;

	/*
	 * Verifies and consumes the auth ticket, then records this instance's
	 * session, all in one atomic step.
	 * Returns 1 admitted / -2 session already held / 0 ticket absent / -1 ticket mismatch.
	 */
	constexpr const char *CLAIM_SESSION_SCRIPT =
		"if redis.call('EXISTS', KEYS[2]) == 1 then return -2 end "
		"local ticket = redis.call('GET', KEYS[1]) "
		"if not ticket then return 0 end "
		"if ticket ~= ARGV[1] then return -1 end "
...
void ChatServer::kick_account(std::int64_t account_no) noexcept
{
	Job *job;

	job = this->job_pool.create(JobType::KickAccount, Job::Target{.account_no = account_no});
	if (!job)
	{
		std::fprintf(stderr, "ChatServer::kick_account(): job_pool.create() failed (out of memory). account: %lld\n", static_cast<long long>(account_no));
		std::terminate();
	}

	if (!this->job_queue.enqueue(job))
	{
		std::fprintf(stderr, "ChatServer::kick_account(): job_queue.enqueue() failed (out of memory). account: %lld\n", static_cast<long long>(account_no));
		std::terminate();
	}
...
void ChatServer::on_client_join(SessionID id) noexcept
{
	...
	job = this->job_pool.create(JobType::ClientJoin, Job::Target{.session_id = id});
	if (!job)
	{
...
void ChatServer::on_client_leave(SessionID id) noexcept
{
	...
	job = this->job_pool.create(JobType::ClientLeave, Job::Target{.session_id = id});
	if (!job)
	{
...
void ChatServer::on_recv(SessionID id, std::unique_ptr<Packet> packet) noexcept
{
	...
	job = this->job_pool.create(JobType::Recv, Job::Target{.session_id = id}, std::shared_ptr<Packet>(std::move(packet)));
	if (!job)
	{
...
ChatServer::Job::Job(JobType type, Target target, std::shared_ptr<Packet> packet) noexcept
	: type(type)
	, target(target)
...
{
	server->redis = connect_redis();
	server->clear_all_session();
...
void ChatServer::drain_jobs(void) noexcept
{
	while (true)
	{
		...
		case JobType::ClientJoin:
			this->handle_client_join(job->target.session_id);
			break;
		case JobType::ClientLeave:
			this->handle_client_leave(job->target.session_id);
			break;
		case JobType::Recv:
			this->handle_recv(job->target.session_id, job->packet.get());
			break;
		case JobType::KickAccount:
			this->handle_kick_account(job->target.account_no);
			break;
		}
...
void ChatServer::handle_client_leave(SessionID id) noexcept
{
	...
	if (player->logged_in)
	{
		...
		this->clear_session_state(player->account_no);
	}
...
void ChatServer::handle_recv(SessionID id, Packet *packet) noexcept
{
	...
	{
	case PacketType::CHAT_REQ_LOGIN:
		this->handle_req_login(player, packet);
		break;
	case PacketType::CHAT_REQ_SECTOR_MOVE:
		this->handle_req_sector_move(player, packet);
		break;
	case PacketType::CHAT_REQ_MESSAGE:
		this->handle_req_message(player, packet);
		break;
	case PacketType::CHAT_REQ_HEARTBEAT:
...
void ChatServer::handle_kick_account(std::int64_t account_no) noexcept
{
	for (const std::pair<const SessionID, Player *> &entry : this->player_map)
	{
		if (!entry.second->logged_in || entry.second->account_no != account_no)
			continue;

		this->logger->log(
			ajy::utility::Logger::LogLevel::Info,
			"handle_kick_account(): kicking. account: %lld, id: %llu",
			static_cast<long long>(account_no),
			entry.first);
		this->disconnect(entry.first);
		return;
	}
}
...
{
	std::uint8_t session_key[SESSION_KEY_SIZE];
	...
	*packet >> player->account_no >> player->id >> player->nickname >> session_key;

	if (!this->claim_session(player->account_no, session_key))
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Warning,
			"handle_req_login(): admission denied. account: %lld, id: %llu",
			static_cast<long long>(player->account_no),
			player->session_id);
		this->disconnect(player->session_id);
		return;
	}
...
bool ChatServer::claim_session(std::int64_t account_no, const std::uint8_t *session_key) noexcept
{
	redisReply *reply;
	bool admitted;
	char ticket_key[REDIS_KEY_SIZE];
	char state_key[REDIS_KEY_SIZE];
	char mine_hex[HEX_PREFIX_BYTES * 2 + 1];

	std::snprintf(
		ticket_key,
		sizeof(ticket_key),
		"%.*s:%.*s:%lld",
		static_cast<int>(ChatServerConfig::TICKET_SECTION.size()),
		ChatServerConfig::TICKET_SECTION.data(),
		static_cast<int>(ChatServerConfig::INSTANCE_NAME.size()),
		ChatServerConfig::INSTANCE_NAME.data(),
		static_cast<long long>(account_no));
	...
	std::snprintf(
		...
		sizeof(state_key),
		"%.*s:%.*s:%lld",
		static_cast<int>(ChatServerConfig::SESSION_SECTION.size()),
		ChatServerConfig::SESSION_SECTION.data(),
		static_cast<int>(ChatServerConfig::INSTANCE_NAME.size()),
		ChatServerConfig::INSTANCE_NAME.data(),
		static_cast<long long>(account_no));
	...
	reply = static_cast<redisReply *>(::redisCommand(
		...
		ticket_key,
		state_key,
		reinterpret_cast<const char *>(session_key),
		SESSION_KEY_SIZE));
	...
	if (!reply)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"claim_session(): EVAL failed: %s. account: %lld",
			this->redis->errstr,
			static_cast<long long>(account_no));
		return false;
	}
	...
	if (reply->type != REDIS_REPLY_INTEGER)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"claim_session(): unexpected EVAL reply type %d. account: %lld",
			reply->type,
			static_cast<long long>(account_no));
	...
	admitted = reply->integer == 1;

	if (!admitted)
	{
		this->to_hex_prefix(mine_hex, session_key, SESSION_KEY_SIZE);
		...
		case -2:
			this->logger->log(
				ajy::utility::Logger::LogLevel::Warning,
				"claim_session(): session already held. account: %lld, mine: %s",
				static_cast<long long>(account_no),
				mine_hex);
			break;
		...
		case 0:
			this->logger->log(
				ajy::utility::Logger::LogLevel::Warning,
				"claim_session(): ticket absent. account: %lld, mine: %s",
				static_cast<long long>(account_no),
				mine_hex);
			break;
		...
		case -1:
			this->logger->log(
				ajy::utility::Logger::LogLevel::Warning,
				"claim_session(): ticket mismatch. account: %lld, mine: %s",
				static_cast<long long>(account_no),
				mine_hex);
			break;
		...
		default:
			this->logger->log(
				ajy::utility::Logger::LogLevel::Error,
				"claim_session(): unexpected EVAL return %lld. account: %lld",
				reply->integer,
				static_cast<long long>(account_no));
			break;
		}
...
void ChatServer::clear_session_state(std::int64_t account_no) noexcept
{
	redisReply *reply;
	char state_key[REDIS_KEY_SIZE];
	...
	reply = static_cast<redisReply *>(::redisCommand(this->redis, "DEL %s", state_key));
	if (!reply)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"clear_session_state(): DEL failed: %s. account: %lld",
			this->redis->errstr,
			static_cast<long long>(account_no));
		return;
	}
	...
	if (reply->type != REDIS_REPLY_INTEGER)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"clear_session_state(): unexpected DEL reply type %d. account: %lld",
			reply->type,
			static_cast<long long>(account_no));
	}
	else if (!reply->integer)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Warning,
			"clear_session_state(): session state absent. account: %lld",
			static_cast<long long>(account_no));
	}
...
void ChatServer::clear_all_session(void) noexcept
{
	redisReply *reply;
	char pattern[REDIS_KEY_SIZE];
	...
	reply = static_cast<redisReply *>(::redisCommand(this->redis, "KEYS %s", pattern));
	if (!reply)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"clear_all_session(): KEYS failed: %s",
			this->redis->errstr);
		return;
	}
	...
	if (reply->type != REDIS_REPLY_ARRAY)
	{
		this->logger->log(
			ajy::utility::Logger::LogLevel::Error,
			"clear_all_session(): unexpected KEYS reply type %d.",
			reply->type);
		::freeReplyObject(reply);
		return;
	}
	...
	for (std::size_t i = 0; i < reply->elements; ++i)
	{
		...
		del_reply = static_cast<redisReply *>(::redisCommand(this->redis, "DEL %s", reply->element[i]->str));
		if (!del_reply)
		{
			this->logger->log(
				ajy::utility::Logger::LogLevel::Error,
				"clear_all_session(): DEL failed: %s. key: %s",
				this->redis->errstr,
				reply->element[i]->str);
			break;
		}
	...
	if (reply->elements > 0)
		this->logger->log(
			ajy::utility::Logger::LogLevel::Warning,
			"clear_all_session(): cleared %zu leftover session states.",
			reply->elements);

	::freeReplyObject(reply);
}

void ChatServer::to_hex_prefix(char *dst, const std::uint8_t *key, std::size_t len) noexcept
{
	static constexpr char DIGITS[17] = "0123456789abcdef";

	std::size_t count;

	count = len < HEX_PREFIX_BYTES ? len : HEX_PREFIX_BYTES;

	for (std::size_t i = 0; i < count; ++i)
	{
		dst[i * 2] = DIGITS[key[i] >> 4];
		dst[i * 2 + 1] = DIGITS[key[i] & 0x0F];
	}
...
std::shared_ptr<ChatServer::Packet> ChatServer::make_res_login(std::uint8_t status, std::int64_t account_no) noexcept
{
	...
	type = static_cast<std::uint16_t>(PacketType::CHAT_RES_LOGIN);
...
std::shared_ptr<ChatServer::Packet> ChatServer::make_res_sector_move(std::int64_t account_no, std::uint16_t x, std::uint16_t y) noexcept
{
	...
	type = static_cast<std::uint16_t>(PacketType::CHAT_RES_SECTOR_MOVE);
...
std::shared_ptr<ChatServer::Packet> ChatServer::make_res_message(const Player *sender, const wchar_t *msg, std::uint16_t msg_len) noexcept
{
	...
	type = static_cast<std::uint16_t>(PacketType::CHAT_RES_MESSAGE);
...
redisContext *ChatServer::connect_redis(void) noexcept
{
	redisContext *context;

	context = ::redisConnect(ChatServerConfig::REDIS_HOST.data(), ChatServerConfig::REDIS_PORT);
	if (!context)
	{
		std::fprintf(stderr, "ChatServer::connect_redis(): redisConnect() failed (allocation).\n");
		std::terminate();
	}

	if (context->err)
	{
		std::fprintf(stderr, "ChatServer::connect_redis(): redisConnect() failed: %s\n", context->errstr);
		::redisFree(context);
		std::terminate();
	}
