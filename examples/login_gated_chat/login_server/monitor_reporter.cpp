 ...
 * Path: ajylib/examples/login_gated_chat/login_server/monitor_reporter.cpp
 * Description:
 *	Drives a Monitor on a 1-second thread and reports the login server's
 *	metrics (DataType 1-6) to the monitoring server as an SS reporter.
 * Note:
...
#include <login_gated_chat/login_server/monitor_report_config.hpp>
...
MonitorReporter::MonitorReporter(ajy::utility::monitor::Monitor &monitor, LoginServer &server, std::string_view logger_name) noexcept
	: monitor(monitor)
	, server(server)
...
bool MonitorReporter::send_login(void) noexcept
{
	...
	packet << type << MonitorReportConfig::LOGIN_SERVER_NO;
...
void MonitorReporter::report_metrics(void) noexcept
{
	...
	const Metric metrics[] = {
		{MonitorReportConfig::MonitorDataType::LOGIN_SERVER_RUN, 1},
		{MonitorReportConfig::MonitorDataType::LOGIN_SERVER_CPU, this->probe_value("process_cpu")},
		{MonitorReportConfig::MonitorDataType::LOGIN_SERVER_MEM, this->probe_value("process_mem_mb")},
		{MonitorReportConfig::MonitorDataType::LOGIN_SESSION, static_cast<std::int32_t>(this->server.get_session_count())},
		{MonitorReportConfig::MonitorDataType::LOGIN_AUTH_TPS, static_cast<std::int32_t>(this->server.get_auth_tps())},
		{MonitorReportConfig::MonitorDataType::LOGIN_PACKET_POOL, static_cast<std::int32_t>(this->server.get_packet_pool_in_use())},
	};
