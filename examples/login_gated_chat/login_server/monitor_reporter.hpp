 ...
 * Path: ajylib/examples/login_gated_chat/login_server/monitor_reporter.hpp
 * Description:
 *	Drives a Monitor on a 1-second thread and, on the same cadence, reports
 *	the login server's metrics (DataType 1-6) to the monitoring server as an
...
#ifndef LOGIN_SERVER_MONITOR_REPORTER_HPP
#define LOGIN_SERVER_MONITOR_REPORTER_HPP
...
{
public:
	MonitorReporter(ajy::utility::monitor::Monitor &monitor, LoginServer &server, std::string_view logger_name) noexcept;
