 ...
 * Path: ajylib/examples/chat_server/single_thread/chat_server.cpp
 * Description:
 *	Definition of ChatServer (declared in chat_server.hpp): the server
 *	lifecycle (content thread spawn/join). Worker callbacks, the content
 *	thread loop, job/packet handlers, sector utilities and response builders
...
{
	this->send_packet(id, this->make_res_login(status, account_no));
}

void ChatServer::send_res_sector_move(SessionID id, std::int64_t account_no, std::uint16_t x, std::uint16_t y) noexcept
{
	this->send_packet(id, this->make_res_sector_move(account_no, x, y));
}
...
void ChatServer::broadcast_res_message(const Player *sender, const wchar_t *msg, std::uint16_t msg_len) noexcept
{
	...
	for (std::size_t i = 0; i < AROUND_COUNT; ++i)
	{
		...
		for (Player *target : this->sectors[around[i]].players)
			this->send_packet(target->session_id, packet);
	}
}
