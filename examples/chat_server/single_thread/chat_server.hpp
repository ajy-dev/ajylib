 ...
 * Path: ajylib/examples/chat_server/single_thread/chat_server.hpp
 * Description:
 *	A sector-based chat server built on ajy::network::windows::iocp::NetServer.
 *	Single-threaded logic model: IOCP workers enqueue jobs; one content
 *	thread owns all shared state and processes them.
 * Note:
...
{
public:
	explicit ChatServer(std::string_view logger_name) noexcept;
